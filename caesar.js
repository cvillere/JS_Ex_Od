const caesar = function(initialString, shiftNum) {
    ...
    const shiftStringArray = initIndexArray.map(function(index){
        ...
        if (shiftNum >= 0) {
            totalLoops = index + shiftNum;
        }
        else if (shiftNum < 0) {
            totalLoops = index - shiftNum;
        }

        finalIndex = shiftNum % 26;

        if (26 > totalLoops >= 0 && shiftNum >= 0){
            for (let i = 0; i <= 25; i++) {
        ...
        else if (totalLoops >= 26 && shiftNum >= 0) {
            for (let i = 0; i <= 25; i++) {
                if (alphabetArray[i] === alphabetArray[finalIndex]){
                    return alphabetArray[i];
                };
            };
        }
        else if (26 > totalLoops >= 0 && shiftNum < 0) {
            for (let i = 0; i <= 25; i++) {
                if (alphaReverseArray[i] === alphaReverseArray[totalLoops]){
